...
test_basic(void)
{
	...
	section1 = tini_section(&ctx, "section1", TINI_STRLEN);
	mu_assert_ptr_ne(section1, NULL);

	node = tini_key(&ctx, global, "global1", TINI_STRLEN);
	...
	node = tini_key(&ctx, global, "global2", TINI_STRLEN);
	...
	node = tini_key(&ctx, section1, "name", TINI_STRLEN);
...
test_positions(void)
{
	...
	node = tini_section(&ctx, "foo", TINI_STRLEN);
...
test_types(void)
{
	...
	section = tini_section(&ctx, "strings", TINI_STRLEN);
	node = tini_key(&ctx, section, "value1", TINI_STRLEN);
	mu_assert_str_eq(tini_str(&ctx, tini_value(node), sval, sizeof(sval)), "test");

	section = tini_section(&ctx, "booleans", TINI_STRLEN);
	node = tini_key(&ctx, section, "value1", TINI_STRLEN);
	mu_assert_int_eq(tini_bool(&ctx, tini_value(node), &bval), 0);
	mu_assert_int_eq(bval, true);
	node = tini_key(&ctx, section, "value2", TINI_STRLEN);
	mu_assert_int_eq(tini_bool(&ctx, tini_value(node), &bval), 0);
	mu_assert_int_eq(bval, true);
	node = tini_key(&ctx, section, "value3", TINI_STRLEN);
	mu_assert_int_eq(tini_bool(&ctx, tini_value(node), &bval), 0);
	mu_assert_int_eq(bval, true);
	node = tini_key(&ctx, section, "value4", TINI_STRLEN);
	mu_assert_int_eq(tini_bool(&ctx, tini_value(node), &bval), 0);
	mu_assert_int_eq(bval, false);
	node = tini_key(&ctx, section, "value5", TINI_STRLEN);
	mu_assert_int_eq(tini_bool(&ctx, tini_value(node), &bval), 0);
	mu_assert_int_eq(bval, false);
	node = tini_key(&ctx, section, "value6", TINI_STRLEN);
	mu_assert_int_eq(tini_bool(&ctx, tini_value(node), &bval), 0);
	mu_assert_int_eq(bval, false);

	section = tini_section(&ctx, "ints", TINI_STRLEN);
	node = tini_key(&ctx, section, "value1", TINI_STRLEN);
	...
	section = tini_section(&ctx, "doubles", TINI_STRLEN);
	node = tini_key(&ctx, section, "value1", TINI_STRLEN);
...
test_setm(void)
{
	...
	section = tini_section(&ctx, "strings", TINI_STRLEN);
	mu_assert_int_eq(tini_setm(&ctx, section, &result, value1), TINI_SUCCESS);
	mu_assert_str_eq(result.value1, "test");

	section = tini_section(&ctx, "booleans", TINI_STRLEN);
	mu_assert_int_eq(tini_setm(&ctx, section, &result, value2), TINI_SUCCESS);
	mu_assert_int_eq(result.value2, true);

	section = tini_section(&ctx, "ints", TINI_STRLEN);
	mu_assert_int_eq(tini_setm(&ctx, section, &result, value3), TINI_SUCCESS);
	mu_assert_int_eq(result.value3, 0xffff);
}
